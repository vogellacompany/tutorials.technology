...
Besides the `tasks` task Gradle also provides a `projects`
...
The `core`, `ui` and `util` sub projects can also have their own _build.gradle_ file, if they have specific needs, which are not already applied by the general configuration of the root project.

For instance the ui project usually has a dependency to the core project.
So the ui project needs its own _build.gradle_ file to specify this dependency.
