...
<3> enables experimental features, like button and menu paths
<4> adds description to the HTML output
<5> adds keywords to the HTML output
<6> Defines the default image location
...
Additional paragraphs, blocks, images, source code, etc. are adjoined to a list element by putting
a plus sign (+) on a line adjacent to both blocks.
