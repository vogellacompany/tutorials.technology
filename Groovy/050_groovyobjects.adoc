...
_Plain Old Groovy Objects (POGO)_.
...
`final`, in this case Groovy will not provide a setter.

[[groovyobjects_constructors]]
=== Constructors

Groovy provides
_constructors with named parameters_

in which you can
specify the
element you would like to set during
construction. This constructor is also called
_map based constructor_, as it uses the
`property:value`
map syntax.
...
The usage of the constructors with named parameters is
...
One difference between Java and Groovy is that the == operator
...
Groovy allows to have optional parameter values. Optional
...
In Groovy you assign default values to parameters in a
method. If
