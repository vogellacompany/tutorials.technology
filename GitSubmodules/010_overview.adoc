...
* Delete the relevant section from the `.gitmodules`  and the `.git/config` file.
* Run `git rm --cached path_to_submodule` (no trailing slash).
* Run `rm -rf .git/modules/path_to_submodule` (no trailing slash).
* Stage the .gitmodules changes via `git add .gitmodules`
* Commit the staged changes with `git commit -m "Removed submodule "`

This removed the submodule.
You can now delete the untracked submodule files with `rm -rf path_to_submodule`.
