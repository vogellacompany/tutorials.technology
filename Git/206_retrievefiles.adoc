...
[source,terminal] ---- include::./examples/show/showrev.txt[] ---- ##
Restore a deleted file in a Git repo \{#retrievefiles_fromcommit}
...
[source,terminal] ----
include::./examples/restore/restoredeletedfile.txt[] ---- ## See which
commit deleted a file \{#retrievefiles_finddeletedfile}

((Find commit which deleted a file))

((File deletion log))
 (((git
log)))

 LARSSECONDARY"file deletion|"LARSSECONDARY The
`git log` command allows you to determine which commit deleted a file.
You can use the `--` option in `git log` to see the commit history for a
file, even if you have deleted the file.

[source,terminal] ---- include::./examples/show/delete.txt[] ----
