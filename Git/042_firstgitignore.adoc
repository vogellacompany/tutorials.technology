...
[source,terminal] ---- include::./examples/remove/gitignorecreate.txt[]
---- > *Tip* > > The above command creates the file via the command
...
[source,terminal] ---- include::./examples/remove/gitignore.txt[] ----
## Stop tracking files based on the .gitignore file \{#untrack}

((.gitignore))
 LARSSECONDARY"remove
already committed files"LARSSECONDARY Files that are tracked by Git are
...
[source,terminal] ---- include::./examples/remove/remove.txt[] ----
...
have a look at `git                 filter-branch` which allows you to
rewrite the commit history. See ? for details.
