...
[source,terminal] ---- include::./examples/merge/createconflict1.txt[]
---- [source,terminal] ----
include::./examples/merge/createconflict2.txt[] ---- [source,terminal]
---- include::./examples/merge/createconflict3.txt[] ---- As this push
would not result in a non-fast-format merge, you receive an error
message similar to the following listing.

[source,terminal] ---- include::./examples/merge/createconflict4.txt[]
---- To solve this, you need to integrate the remote changes into your
local repository. In the following listing the `git fetch` command gets
the changes from the remote repository. The `git merge` command tries to
integrate it into your local repository.

[source,terminal] ---- include::./examples/merge/createconflict5.txt[]
---- This creates the conflict and a message similar to the following.

[source,terminal] ---- include::./examples/merge/createconflict7.txt[]
...
[source,terminal] ---- include::./examples/merge/mergeconflict.txt[]
---- The text above the ======= signs is the conflicting change from
your current branch and the text below is the conflicting change from
...
[source,terminal] ----
include::./examples/merge/mergeconflictsolved.txt[] ---- Afterwards add
the affected file to the staging area and commit the result. This
creates the merge commit. You can also push the integrated changes now
to the remote repository.

[source,terminal] ---- include::./examples/merge/createconflict6.txt[]
