...
(((Commit process)))
After adding the selected files to the staging area, you can _commit_ these files to add them permanently to the Git repository. 
_Committing_ creates a new persistent snapshot (called _commit_ or _commit object_) of the staging area in the Git repository. 
...
For example, you can work independently on a branch called _production_ for bugfixes and on another branch called `feature_123` for implementing a new feature. 
...
Local branches can be compared with other local branches and with _remote-tracking_ branches.
...
Selecting a branch in Git terminology is called _to checkout_ a branch. 
